...
# Set a directory to save image files 
PlotPath <- "./plotting"
dir.create(PlotPath)
...
# Define a function to save image files
ImageSave_fn <- function(ImageObject, Name) {
    jpeg(file=(paste0(PlotPath, Name)))
    ImageObject
    dev.off()
}
...
# Save the plot
ImageSave_fn(LibSizePlot, "/LibSizePlot.jpg")
...
# Save the plot
ImageSave_fn(TranscriptDistPlot, 
             "/TranscriptDistPlot.jpg")


```

## Cleaning TPM data prior to DE analysis
### Converting the raw TPM to an integer matrix
...
LFCTable_fn <- function(df) {
    ...
    df <- df %>% 
        ...
        mutate(Significance=ifelse(padj < 0.1 & !is.na(padj), 
                                   "Significant", 
                                   "Non-significant")) 
...
FDR_stat <- data.frame(NumOfTx=nrow(shRes),
                       NumOfSigTx=sum(shRes$Significance == "Significant"),
                       NumOfNAFDR=sum(is.na(shRes$padj)), 
                       PropOfSigTx=sum(shRes$Significance == "Significant") / nrow(shRes))


head(shRes[shRes$Significance == "Significant",])
```
