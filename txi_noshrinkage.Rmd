...
for (x in TvC) {
    ...
    normal.ddsList <- lfcShrink(ddsList[[x]],
                                contrast=Contrast,
                                type="normal")

    ape.ddsList <- lfcShrink(ddsList[[x]],
                             coef=resultsNames(ddsList[[x]])[2],
                             type="apeglm")

    ash.ddsList <- lfcShrink(ddsList[[x]],
                             coef=resultsNames(ddsList[[x]])[2],
                             type="ashr")
...
for (x in TvC) {
    ...
    MAList[[x]] <- ggplot(resList[[x]], 
                         aes(x=baseMean,
                             y=log2FoldChange,
                             color=FDR)) + geom_point() + scale_x_log10() + theme_bw() + scale_color_manual(values=c("blue", "grey")) + ggtitle(paste("MA plot:", x)) + ylim(yl[1], yl[2]) + geom_hline(yintercept=c(mLog[1], mLog[2]), linetype="dashed", color="red")

}

# Print MA plots
grid.arrange(MAList[[1]], MAList[[2]], ncol=2)
...
#### - Both inputs showed similar shape of FDR distribution overall

#### - TPM input found more genes in FDR > 0.7 

#### - Count input found more genes in FDR < 0.3
...
#### - Count inputs found more genes between -1 and 1 log2FoldChange 


## Exploring expression profiling with normalized count data

#### - Normalized count matrices are extracted from dds objects and filtered by with thresholds set at FDR and log2FoldChange
