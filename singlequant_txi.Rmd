...
grid.arrange(QCcorrHeatmap_fn(TPM, "TPM"), 
            (QCcorrHeatmap_fn(Counts, "Counts")), 
             nrow=1) 
...
- Shrinkage helping reducing false positives 
- Magnitude of shrinkage is affected by dispersion and sample size
- When the **type="apeglm"**, the **coef** argument is used instead of constrast. In this dataset, you can set **coef=Coef** where **Coef <- resultsNames(ddsList[[1]])**. 
...
for (i in 1:2) {
    ...
    # shrink
    shr_ddsList[[i]] <- lfcShrink(ddsList[[i]], 
...
for (i in 1:2) {
    ...
    # Extract results
    resList[[i]] <- as.data.frame(results(ddsList[[i]], 
                                          contrast=Contrast, 
                                          alpha=alpha))

    shr_resList[[i]] <- as.data.frame(shr_ddsList[[i]])

    # clean the data frame
    resList[[i]] <- Sig_fn(resList[[i]], names(ddsList)[i])
    shr_resList[[i]] <- Sig_fn(shr_resList[[i]], names(ddsList)[i])
...
ggtitle(paste("MA plot:", names(List)[i], "Input with", Shr)) + 
ylim(yl[1], yl[2])
...
- Red dashed line: log2FoldChange = 1 
...
- Black dashed line: log2FoldChange = 1
...
- This normalized and filtered count matrices will be used to as inputs for creating profiling heatmaps


```{r transcription_profiling_FDR}


# Define the minimum threshold for log2FoldChange 
minLog <- 1
...
for (i in 1:2) {
    ...
    # Create vectors of TRUE/FALSE filtering alpha and log2foldchange
    BelowAlpha <- resList[[i]]$FDR == "< 0.1"
    overminLog <- BelowAlpha & resList[[i]]$log2FoldChange > minLog
    
    # Extract normalized counts from dds objects with counts() 
    normCounts <- counts(ddsList[[i]], 
                            normalized=T)

    # Update the normalized count matrix with FDR below alpha
    lowfdrList[[i]] <- normCounts[BelowAlpha, ]
    highfoldList[[i]] <- normCounts[overminLog, ]

    summary(lowfdrList[[i]])
    summary(highfoldList[[i]])
}

```

## Creating heatmaps for expression profiling 
...
ProfileHeatmap_fn <- function(inputmatrix, Title) {
...
for (i in 1:2) {
    ...
    lowfdrMap[[i]] <- ProfileHeatmap_fn(lowfdrList[[i]], 
                                        paste("FDR <", alpha))
    highfoldMap[[i]] <- ProfileHeatmap_fn(highfoldList[[i]], 
                                          paste("FDR <", 
                                                alpha, 
                                                "Plus Minimum Log2 Fold Change >", minLog))
}
