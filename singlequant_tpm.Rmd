...
TPMTable <- data.frame(Gene=AnnoDb$SYMBOL)
...
LibSizeTable <- colSums(TPMTable[2:ncol(TPMTable)], 
                        na.rm=TRUE) 
...
TranscriptDistPlot <-
    ggplot(txDistribution,
       aes(x=Read,
           ...
           xlab("Number of Transcripts per Gene (TPM)") +
           ylab("Density") + 
           scale_x_log10()
